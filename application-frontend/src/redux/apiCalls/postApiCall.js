...
// Fetch Posts Based On Searc hText
export function fetchPostsBasedOnSearchText(searchText) {
  return async (dispatch) => {
    try {
      const { data } = await request.get(`/api/posts?searchText=${searchText}`);
      dispatch(postActions.setPostsWithSearchText(data));
    } catch (error) {
      toast.error(error.response.data.message);
    }
  };
